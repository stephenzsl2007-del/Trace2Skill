...
def main() -> int:
    ...
    parser.add_argument("failed_trace", type=Path)
    parser.add_argument("--worker", default="trace-worker")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    failed = json.loads(args.failed_trace.read_text(encoding="utf-8"))
    fixtures = FixtureCatalog(ROOT / "product" / "fixtures").load()
    fixture = fixtures[failed["fixture_id"]]
    ...
    trace = runner.recover(
        fixture,
        worker,
        failed["task_id"],
        failed.get("assignment"),
        failed["started_at"],
    )
    ...
    print(json.dumps({"trace": str(output), "status": trace["status"]}, ensure_ascii=False))
